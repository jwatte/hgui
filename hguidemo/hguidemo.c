...
typedef struct twoWidgets { hWidget *w1,*w2;} twoWidgets;
...
{ setTitle(getWin(w),"Button 1 pressed!"); butt2->state=0; butt2->title="b1press"; butt2->x1+=1; butt2->x2+=1; redrawWidget(butt2); } 

void b2callback(ButtonDyn *butt2) 
{ setTitle(getWin(butt2),"Button 2 pressed!"); butt2->title="Na?"; redrawWidget(butt2); } 

void b3callback(hWidget *w, hWidget *scr1) 
{ setTitle(getWin(w),"Button 3 pressed!"); scr1->pos=0.2; redrawWidget(scr1); toggleFullscreen(getWin(w)); }

void sl1callback(hWidget *slider1, char* constr) 
{ hWindow *win=getWin(slider1); setTitle(win,"Slider1"); setContrast(win,slider1->pos); 
  sprintf(constr,"c.:%1.1f",getContrast(win)); changeTextPtr(slider1,constr); } //slider1->caption=constr; redrawWidget(slider1); }

void hslid1callback(hWidget *sl1, hWidget* prog3) 
{ hWindow *win=getWin(sl1); setTitle(win,"horizontal Slider callback"); 
  ...
  if (eventButton(win)==SCROLLDOWN) {prog3->pos-=0.01; if(prog3->pos<=0.0) prog3->pos=0.0; redrawWidget(prog3);}
  else if (eventButton(win)==SCROLLUP) {prog3->pos+=0.1; redrawWidget(prog3);} };

void scrcallback(hWidget *w, hWindow *win) { setTitle(win,"Scrolling ás, ő maga ");  }

void piccallback(hWidget *pic, twoWidgets *tw) 
{ hWidget *prog1=tw->w1; hWidget *xpb1=tw->w2; setTitle(getWin(pic),"Pictureclick"); prog1->pos+=0.01; 
  if(prog1->pos>1.0) prog1->pos=0.05; redrawWidget(prog1); xpb1->data=mouse_xpm; redrawWidget(xpb1);  }

void xpmcallback(PictureXPM *xpm, twoWidgets *t) 
{ hWidget *prog1=t->w1; hWidget *xpb1=t->w2; prog1->pos-=0.02; 
  if(prog1->pos<0) prog1->pos=0.9; redrawWidget(prog1); xpb1->data=icon_xpm; redrawWidget(xpb1); }

void butxpmcallback(hWidget *w, char* toggle) 
{ setIconXPM(getWin(w),*toggle?questionmark_xpm:mouse_xpm); *toggle^=1; setTitle(getWin(w),"ProgramIcon changed"); }

void prog2callback(hWidget *pr2, hWindow *win) 
{ if(eventButton(win)==SCROLLUP) pr2->pos+=0.1; if(eventButton(win)==SCROLLDOWN) pr2->pos-=0.2; 
  if(eventButton(win)==LEFTBUTTON) pr2->pos=(float)(eventX(win)-pr2->actx1)/(float)(pr2->actx2-pr2->actx1); 
  correctRange(&pr2->pos); redrawWidget(pr2); }

void prog3callback(hWidget *prog3, hWindow *win) 
{ if(eventButton(win)==SCROLLUP) prog3->pos+=prog3->ratio; if(eventButton(win)==SCROLLDOWN) prog3->pos-=prog3->ratio; redrawWidget(prog3); }

void subwbcallback() { printf("Subwindow button pressed\n"); }

void loopcallback(hWindow *hw, char* fullscrswitch) {if(*fullscrswitch==0 && getWinWidth(hw)>720) {setTitle(hw,"Bigger than 720p!"); *fullscrswitch=1;} }

void exitcallback(hWindow *hw) { printf("Exit button on main window pressed: Exiting by this custom callback-function...\n"); destroyWindow(hw); exit(0);  }
void childExit(hWindow *hw) { printf("Exit button on child-window pressed. Killing child-window \"%s\" by custom callback.\n",getTitle(hw)); destroyWindow(hw); }
...
int main (int argc, char **argv)    //note: literals (buttun-names,progressbar-captions,etc.) get destroyed when main exits.
{
 ...
 ButtonDyn *butt2; HProgBar *prog2; ButtonXPM *xpb1;
 hWidget *text1,*slider1,*prog1,*prog3,*scr1; PictureXPM *xpmw; 
 ...
 addBlockFill(hwin,250,270,330,-50,0x251506); addTextCentered(hwin,250,270,330,-50,"CenText ógé",0xf0e0a0,"fixed");
 addHScroll(hwin,440,-25,-12,16,0x3a6302,0x002000,0x40c020,0.18,0.74,scrcallback,hwin); 
 int w=124,h=98; char pixeldata[w*h*32/8]; for(i=0;i<w*h*4;i++) pixeldata[i]=rand()*255; 
 ...
 twoWidgets twoW={prog1,(hWidget*)xpb1}; addPicture(hwin,100,-180,w,h,32,pixeldata,piccallback,&twoW); 
 ...
 addPoint(hwin,1,1,0x305020); drawLine(hwin,10,10,36,26,0xf00010); addLine(hwin,20,10,50,26,0x0000F0);
 addCircle(hwin,320,150,26,0x150260); addCircleFill(hwin,320,150,15,FGCOLOR); 
 addEllipse(hwin,-90,77,32,16,0x558260); addEllipseFill(hwin,-90,77,17,12,0x35A240);
 addRect(hwin,-40,65,-10,83,0); addRectFill(hwin,-36,70,-15,79,0xc08090);
 addArc(hwin,-50,-50,-1,-1,270,90,0x0000c0); addArcFill(hwin,310,-76,350,-25,300,50,0x80a0c0);
 ...
 setContrast(childwin,0.15); moveWindow(childwin,ChildX-60,ChildY-40); //iconifyWindow(childwin); //hideWindow(childwin); //showWindow(hwin);
 setClass(hwin,"HermitGUI_demo","hguidemo"); setClass(childwin,"HermitGUI_demo","hguidemo_childwin");
 addTextBox(childwin,10,10,-80,80,"\niieuiola\nmély\nszőr  aoe\noei a a\noei u keo iio \na\noabsol\n",fadeCol(BGCOLOR,0.2),0x404040,1,1,1,1,4,"fixed"); 
 addTextScroll(childwin,1,-30,-1,-1,scrolltext,BGCOLOR,0,2,1,"fixed"); addButton(childwin,100,100,100,60,"ChildButton",BGCOLOR,FGCOLOR,0,1,1,"6x13",b3callback,scr1);
 ...
 setContrast(subwin,0.8); addButton(subwin,5,5,40,20,"subwin",BGCOLOR,FGCOLOR,0,1,1,"6x13",subwbcallback,NULL); 
