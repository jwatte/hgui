...
int main (int argc, char **argv)
{
 ...
 if (argc<2) { printf("Usage: diagram <inputfile>\n"); return 1; }

 InputFile = fopen(argv[1],"rb"); printf("\n");
 datalen=0;
 do { readata=fgetc(InputFile); data[datalen++]=readata; if(readata!=EOF) printf("%2.2X ",readata); } while (readata!=EOF && datalen<MAX_DATA_LEN);
 datalen--;

 printf("\n%d bytes read \n",datalen); fclose(InputFile); 
