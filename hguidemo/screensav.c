...
{
 unsigned char hacknum=0; int i; //default is blanking
 if (argc==2) 
 { 
  ...
  {
   for(i=0;hacknames[i]!=NULL;i++) if (!strcmp(argv[1],hacknames[i])) {hacknum=i; break; } 
  }
 }
 ...
 setClass(hw,"screensaver","hguiscreensav"); hideMouseCursor(hw); 
...
// ============================= HACKS =================================
#define PI 3.14
int movcnt=0,stripcnt=0; float poscnt=0.0; 
unsigned long anicol=0x405070; char phase=0;
float angle=0.0,anglecnt=0.0; float rad=PI/180.0;
...
void ellipsoid()
{
 ...
 if(movcnt<getWinWidth(hw)/2) { movcnt++;  if(!phase && movcnt%50==0) anicol+=0x104A52; }
 else {movcnt=0; phase^=1; }
}
...
void cube()
{
 int i,j; anicol+=0x010010; float focus=10.0, posx,posy,posz,sizex,sizey;
 for (phase=0;phase<2;phase++)
 {
  posz=10+sin(anglecnt*rad)*5; posx=sin(anglecnt*3*rad)*width/3; posy=cos(anglecnt*5*rad)*height/3; sizex=width/20; sizey=height/20;
  for(i=0;i<8;i++) 
  {
   for(j=0;j<8;j++)
   {
...
void starfield()
{
 for (phase=0;phase<2;phase++)
 {
  ...
  { 
   ...
   drawPoint(hw,width/2+poscnt*sin(poscnt+anglecnt),height/2+poscnt,anicol); drawPoint(hw,width/2+poscnt*sin(poscnt+anglecnt),height/2-poscnt,anicol);
   drawPoint(hw,width/2-poscnt,height/2+poscnt*cos(poscnt+anglecnt),anicol); drawPoint(hw,width/2+poscnt,height/2+poscnt*cos(poscnt+anglecnt),anicol);
  }
...
{
 clearWindow(hw); *hacknum=*hacknum+1; if(hackfunc[*hacknum]==NULL) *hacknum=1; //start over
 //width=getWinWidth(hw); height=getWinHeight(hw);
 movcnt=poscnt=stripcnt=phase=0; angle=anglecnt=0.0;
}

void callhack (hWindow *hw, unsigned char *hacknum)
{
 if(changecnt<=0) { changecnt=TIMEOUT; changeHack(hacknum); } else changecnt--;
