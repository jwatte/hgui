...
typedef struct hWidget { //universal widget structure - full version is defined later in hgui.c, their structure should match exactly to the specific widgets below
 union{int x,x1;} ; union{int y,y1;} ; union{int x2,w,r,rx;} ; union{int y2,h,ry;} ;  
 union{int hpos,border,knobsize,depth,startdeg;} ; union{long vpos,bevel,scalewidth,degree;} ;  
 union{unsigned long bgcol,color,transpcol;} ; union{unsigned long fgcol,textcol;} ; 
 union{unsigned long knobcol,progtextcol;} ; unsigned long captcol;
 union{void *data,*text,*content,*title,*caption,*pixdata,*xpm,*xpmlist;} ;  
 float pos ; union{float ratio,scale;} ; 
 union{char wrap,framedelay;} ; char scrollbars ; union{char pixstep,autofit;} ;
 char state; int actx1,acty1,actx2,acty2; //read-only (writing could fuck up the GUI 
} hWidget;
...
//these distinct widget-structures are not totally necessary (but strongly advised), the universal 'hWidget' structure can be used in their place:
 ...
 typedef struct Button {int x,y,w,h; int border; long bevel; unsigned long color,textcol,nop1,nop2; char* title; float nop3,nop4; char state;} Button; // if bevel>1: 3D-effect
 ...
 typedef struct ButtonDyn {int x1,y1,x2,y2; int border;long bevel; unsigned long color,textcol,nop1,nop2; char* title; float nop3,nop4; char state;} ButtonDyn; // if bevel>1: 3D-effect
...
typedef struct XPMstruct { char *pixdata; int width, height; } XPMstruct; 
